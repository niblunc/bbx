...
def ffq_calculations():
    ...
    kcal_df=ffq_df.filter(like="_kcal", axis=1)
...
def bis_calculation():
    ...
    bis_15_scored= pd.DataFrame(bis_dict).T  
    bis_15_reversed=s1_behav_copy.filter(regex="bis_[0-9]")
    ...
    return bis_15_scored, bis_15_reversed;
...
def fci_calculation():
    ...
    return df_final, s1_behav_copy.filter(like="fci");
...
def debq_calculation():
    ...
    return df_final, s1_behav_copy.filter(like='debq');
...
def spsrq_calculation():
    ...
    return df_final, s1_behav_copy.filter(like="spsrq");
...
def bis_bas_calculation():
    ...
    return df_final, s1_behav_copy.filter('bis_bas');
...
def pfs_calculation():
    ...
    return df_final, s1_behav_copy.filter(like="pfs");
...
def yfas_calculation():
    ...
    for bbx_id in s1_behav.index.values:
        try:
            ...
            yfas_c4=sum(yfas_df.loc[bbx_id, ['%sBx_yfas_8'%sess_id , '%sBx_yfas_9'%sess_id , '%sBx_yfas_10'%sess_id, '%sBx_yfas_11'%sess_id ]])
    ...
    return df_final, s1_behav_copy.filter(like='yfas');
